  ...
  return (
    <section className="px-6 py-14">
      <div
        ...
        style={{ background: "linear-gradient(120deg, var(--brand-cta), var(--brand-cta-deep))" }}
      >
        <div
        ...
        <div className="relative mt-7 flex flex-col items-center gap-3">
          <Link href={signedIn ? "/dashboard" : "/sign-up"}>
            <Button size="lg" variant="outline" className="border-white bg-white text-[var(--brand-cta)] hover:bg-white/90">
              {signedIn ? "Go to dashboard" : "Build your bot free"}
            </Button>
          </Link>
          {!signedIn ? <p className="text-sm text-white">No credit card required</p> : null}
        </div>
      </div>
    </section>
  );
